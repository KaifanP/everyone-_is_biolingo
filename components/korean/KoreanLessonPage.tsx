...
import { KoreanLesson, KOREAN_STEP_LABELS } from "@/lib/korean-types";
...
const MOCK_TEST_STEP = 6;

export default function KoreanLessonPage({ data, moduleTitle }: Props) {
  const [currentStep, setCurrentStep] = useState(0);

  const hasMockTest = !!data.mockTest;
  const totalSteps = hasMockTest ? 7 : 6;
  const CurrentStepComponent = currentStep < 6 ? stepComponents[currentStep] : null;
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="sticky top-0 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg py-4 mb-6 border-b border-gray-100 dark:border-gray-800">
          <KoreanStepNav currentStep={currentStep} onStepChange={setCurrentStep} hasMockTest={hasMockTest} />
          <p className="mt-2 text-center text-xs text-gray-500 dark:text-gray-400">
            {currentStep === MOCK_TEST_STEP && hasMockTest
              ? "在限定时间内完成模拟测试，检验学习成果。"
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {currentStep === MOCK_TEST_STEP && hasMockTest ? (
              <KoreanMockTest mockTest={data.mockTest!} lessonId={data.info.id} />
        ...
        <div className="flex justify-between items-center mt-10 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          >
            ← {currentStep === MOCK_TEST_STEP + 1 ? KOREAN_STEP_LABELS[5].label : KOREAN_STEP_LABELS[Math.max(0, currentStep - 1)]?.label}
          </button>
          ...
          >
            {currentStep === 5 && hasMockTest ? "模拟测试" : KOREAN_STEP_LABELS[Math.min(5, currentStep + 1)]?.label} →
          </button>
        </div>
      </div>
    </main>
